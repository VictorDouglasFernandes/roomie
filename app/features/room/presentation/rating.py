...
class Rating:
    def __init__(self, room=None, controller=None, creator=None):
        self.raiz = Tk()
        ...
        if self.room.ratings:
            for index in range(len(room.ratings)):
                self.create_list_item(index * 2, room.ratings[index])

        self.bottom_frame = Frame(self.raiz, bd=4, bg=kWhite)
        self.bottom_frame.place(relx=0.1, rely=0.9, relheight=0.1, relwidth=0.8)

        if not creator:
            self.back_button = Button(self.bottom_frame, text="Avaliar", bg=kYellow, fg=kWhite,
                                      command=self.read_rating)
    ...
    def read_rating(self):
        rating = askstring('AVALIAÇÃO', 'Digite aqui:')

        def has_rating():
            return rating

        if has_rating():
            if self.controller.add_room_rating(rating, self.room):
                self.show_message("Sucesso", "Avaliação registrada com sucesso!")
                self.back()
            else:
                self.show_message("Erro", "Tente novamente")

    def tela(self):
        self.base = DefaultSingleFrame(self.raiz, "AVALIAÇÕES")
    ...
    def create_list_item(self, row, ratings):
        if len(ratings) > 100:
            ratings = ratings[:100] + '\n' + ratings[100:]

        Label(self.list_frame, text=f"- {ratings}").grid(row=row, column=0, pady=5, padx=5)
