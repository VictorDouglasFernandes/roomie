...
class ListRoomie:
    def __init__(self, roomies=None):
        self.roomies = roomies
        self.navigation = None
        self.selected_id = None
        self.raiz = Tk()
        self.base = DefaultSingleFrame(self.raiz, title="Colegas de Quarto Disponíveis")
    ...
    def see_selected(self):
        if len(self.listbox.curselection()) > 0:
            print(self.roomies[self.listbox.curselection()[0]])
    ...
    def scrollbox(self):
        ...
        for roomie in self.roomies:
            self.listbox.insert(END, f"{roomie.share_date} {roomie.price}")
